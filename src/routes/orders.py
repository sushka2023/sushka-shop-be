...
@router.patch("/change_quantity", response_model=OrderedItemsResponse)
async def change_quantity_items_to_order(
        body: ChangeQuantityOrderItemModel,
        order_id: str = Header(...),
        db: Session = Depends(get_db)
):
    """
    The change_quantity_items_to_order function updates quantity of product in the order of anonym user.

    Args:
        body: ChangeQuantityOrderItemModel: Get the quantity of product from the request body
        order_id: Order: unique identity order of anonym user
    ...
    order_item = await repository_orders.get_order_item_by_id(order.id, body.id, db)
    ...
    update_quantity_order_item = await repository_orders.update_quantity(order_item, body.quantity, db)

    product = await repository_products.product_by_id(order_item.product_id, db)
    ...
    update_quantity_order_item = OrderedItemsResponse(id=update_quantity_order_item.id,
                                                      order_id=(
                                                            update_quantity_order_item.order_id
                                                             ),
                                                      product_id=update_quantity_order_item.product_id,
                                                      product=exist_product,
                                                      quantity=update_quantity_order_item.quantity,
                                                      prices=selected_price,
                                                      price_id=(
                                                            update_quantity_order_item.price_id)
                                                      )

    return update_quantity_order_item


@router.delete("/remove_product", status_code=status.HTTP_204_NO_CONTENT)
async def remove_product(
        body: OrderItemAnonUserRemoveModel,
        order_id: str = Header(...),
        db: Session = Depends(get_db)
):
    """
    The remove_product function removes a product from the basket.
        The function takes in a body of type OrderItemsRemoveModel, which contains the id of the object to be removed.
        Finally, it takes in an optional db parameter, which is used for database access.

    Args:
        order_id: Order: unique identity order of anonym user
        body: OrderItemAnonUserRemoveModel: Get the item id from the request body
        db: Session: Get a database session
    ...
    await repository_orders.remove_item(order_item, db)
