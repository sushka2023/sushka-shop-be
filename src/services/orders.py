...
from src.database.models import (
    Basket, BasketAnonUser, BasketItemAnonUser, OrderedProduct, BasketItem
)
...
async def calculate_basket_total_cost_for_anonym_user(basket: BasketAnonUser):
...
async def move_product_to_ordered_for_anon_user(
        db: Session, basket_item_anon_user: BasketItemAnonUser
) -> OrderedProduct | None:
    ordered_product = OrderedProduct(
...
async def delete_basket_items_by_basket_number_id(basket_anon_user_id: int, db: Session):
    """Anonym User Shopping Basket Cleaning"""
    try:
        db.query(BasketItemAnonUser).filter(BasketItemAnonUser.basket_anon_user_id == basket_anon_user_id).delete()
        db.commit()
    except SQLAlchemyError as e:
        logger.exception("SQLAlchemyError")
        db.rollback()
