...
async def create_postal_office(current_user: User, db: Session):
...
async def add_ukr_postal_office_to_post(update_post: PostAddUkrPostalOffice, post_id: int, user_id: int, db: Session):
...
async def get_posts_by_user_id(user_id: int, db: Session) -> Post | None:
    return db.query(Post).options(joinedload(Post.ukr_poshta)).filter(Post.user_id == user_id).first()


async def get_posts_by_id_and_user_id(post_id: int, user_id: int, db: Session) -> Post | None:
    post = (
        db.query(Post).options(joinedload(Post.ukr_poshta))
